...
import {
  User, LogOut, ChevronDown, Bell, ChevronRight, Shield, X,
  Heart, BookOpen, CreditCard
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

// Unified glassmorphism/gradient style for header
...
// Distinct style for dropdown
const dropdownGlassBg = "bg-white/80 backdrop-blur-lg border border-peach-200 shadow-2xl";
const dropdownGradientAccent = "bg-gradient-to-r from-pink-50 to-peach-100";
...
export const Header = () => {
  ...
  // Updated menu items to reflect actual project features
  const menuItems = [
    {
      icon: <User size={18} />, text: "My Profile", subtext: "View and edit profile", link: "/profile", color: "text-blue-500"
    },
    {
      icon: <Heart size={18} />, text: "Marriage Counseling", subtext: "Professional guidance for couples", link: "/marriage-counseling", color: "text-pink-500"
    },
    {
      icon: <BookOpen size={18} />, text: "Relationship Therapy", subtext: "Expert support for relationships", link: "/relationship-therapy", color: "text-purple-500"
    },
    {
      icon: <Bell size={18} />, text: "Notifications", subtext: "View all alerts", link: "/notifications", color: "text-amber-500"
    },
    {
      icon: <CreditCard size={18} />, text: "Billing", subtext: "Manage your subscription", link: "/billing-process", color: "text-green-500"
    },
    {
      icon: <Shield size={18} />, text: "Privacy", subtext: "Manage your privacy", link: "/privacy", color: "text-gray-500"
    },
    {
            ...
            {notificationDropdownOpen && (
              <>
                ...
                <motion.div
                  ...
                  className={`absolute right-0 mt-2 w-80 ${dropdownGlassBg} ${dropdownGradientAccent} rounded-xl overflow-hidden z-50`}
                >
                  <div className="p-4 border-b border-peach-100 bg-gradient-to-r from-pink-50 to-peach-50">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notification) => (
                        <motion.div
                          ...
                          className={`p-4 border-b border-peach-100 hover:bg-peach-50 cursor-pointer ${!notification.read ? 'bg-peach-50' : ''}`}
              ...
              {dropdownOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className={`absolute right-0 mt-3 w-80 ${dropdownGlassBg} ${dropdownGradientAccent} rounded-xl overflow-hidden z-50`}
                  >
                    <motion.div
                      initial={{ y: -5, opacity: 0 }}
                      animate={{ y: 0, opacity: 1 }}
                      transition={{ delay: 0.1 }}
                      className="px-6 py-5 border-b border-peach-100 bg-gradient-to-r from-pink-50 to-peach-50"
                    >
                      <div className="flex items-center space-x-4">
                        <motion.img
                          src={getAvatar()}
                    ...
                    <div className="py-2">
                      {menuItems.map((item, index) => (
                        <motion.div
                          ...
                          className="px-4 py-3 hover:bg-peach-50 transition-colors duration-150"
                        >
                          {item.link ? (
                            <Link
                              to={item.link}
                    ...
                    <div className="border-t border-peach-100 px-6 py-4 bg-gradient-to-r from-pink-50 to-peach-50">
                      <div className="flex items-center justify-between">
                        ...
                        <Link to="/billing-process">
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="px-4 py-2 bg-gradient-to-r from-pink-500 to-peach-500 text-white text-sm rounded-full font-medium shadow-md hover:from-pink-600 hover:to-peach-600"
                          >
                            Upgrade
                          </motion.button>
                        </Link>
